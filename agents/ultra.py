"""
Ultra agent for Orbit Wars.

Apex infrastructure + key behavioral changes from 1000-score agents:
- Phase detection with adjusted defense ratios
- Threat-based offense skip (hold garrison when threatened)
- Distance penalty in target scoring (avoid overreach)
- Production-weighted early game scoring
"""
...
def _aim_with_prediction(src, target, ships, initial_by_id, ang_vel, comets, comet_ids):
    ...
    is_orbiting = not _is_static(target, initial_by_id) and not is_comet
    ...
    for _ in range(8):
        ...
        if abs(ntx - tx) < 0.3 and abs(nty - ty) < 0.3 and abs(next_est[1] - turns) <= 1:
            ...
            if is_comet and not _validate_comet_intercept(
                ...
                return _search_intercept(src, target, ships, initial_by_id, ang_vel, comets, comet_ids)
            # For orbiting planets with ETA mismatch, try to refine or validate
            if is_orbiting and next_est[1] != turns:
                # Re-predict at the actual arrival ETA
                rpos = _predict_pos(target, initial_by_id, ang_vel, next_est[1])
                if rpos is not None:
                    refined = _estimate_arrival(
                        src.x, src.y, src.radius, rpos[0], rpos[1], target.radius, ships)
                    if refined is not None and refined[1] == next_est[1]:
                        return refined[0], refined[1], rpos[0], rpos[1]
                # Refinement didn't converge; validate with step-by-step sim
                if _validate_orbit_intercept(
                        src, target, next_est[0], next_est[1], ships,
                        initial_by_id, ang_vel):
                    return next_est[0], next_est[1], ntx, nty
                # Validation failed; fall back to brute-force search
    ...
    if is_comet and not _validate_comet_intercept(
        ...
        return None
    # For orbiting planets, validate after loop exhaustion
    if is_orbiting and not _validate_orbit_intercept(
            src, target, final[0], final[1], ships, initial_by_id, ang_vel):
        return _search_intercept(src, target, ships, initial_by_id, ang_vel, comets, comet_ids)
    ...
    for ct in range(1, max_t + 1):
        ...
        if is_comet:
            ...
            if not _validate_comet_intercept(
                ...
                continue
        elif not _is_static(target, initial_by_id) and delta > 0:
            # Skip expensive validation if we already have an exact (delta=0) match
            if best_score is not None and best_score[0] == 0:
                continue
            # Orbiting planet with ETA mismatch — validate with simulation
            if not _validate_orbit_intercept(
                    src, target, confirm[0], confirm[1], ships,
                    initial_by_id, ang_vel):
                continue
...
def _validate_orbit_intercept(src, target, angle, turns, ships, initial_by_id, ang_vel):
    """Verify fleet actually hits an orbiting planet.

    Checks the second half of the flight through 2 steps past predicted arrival.
    Early hits (fleet crossing the orbit ring) can happen well before the
    predicted arrival, so we start from max(1, turns//2).
    """
    speed = _fleet_speed(max(1, int(ships)))
    lx, ly = _launch_pt(src.x, src.y, src.radius, angle)
    cos_a = math.cos(angle)
    ...
    # Compute orbital radius once for quick orbit-proximity check
    init = initial_by_id.get(target.id)
    orbit_r = _dist(init.x, init.y, SUN_X, SUN_Y) if init else 0

    for step in range(max(1, t // 2), t + 3):
        fx0 = lx + cos_a * speed * (step - 1)
        fy0 = ly + sin_a * speed * (step - 1)
        fx1 = lx + cos_a * speed * step
        fy1 = ly + sin_a * speed * step

        # Quick skip: if fleet midpoint is far from the orbit ring, skip
        if orbit_r > 0:
            mx = (fx0 + fx1) * 0.5
            my = (fy0 + fy1) * 0.5
            d_sun = _dist(mx, my, SUN_X, SUN_Y)
            if abs(d_sun - orbit_r) > target.radius + speed + 1.0:
                continue

        px, py = _predict_pos(target, initial_by_id, ang_vel, step)
        d = _point_to_seg_dist(px, py, fx0, fy0, fx1, fy1)
        if d < target.radius:
            return True
...
class World:
    ...
    def __init__(self, player, step, planets, fleets, initial_by_id,
        ...
        # Phase detection
        enemy_prod = sum(p for o, p in self.production.items() if o != player)
        prod_ratio = self.my_prod / max(1, enemy_prod)
        ship_ratio = self.my_total / max(1, self.enemy_total)
        n_my = len(self.my_planets)
        n_enemy = len(self.enemy_planets)

        if prod_ratio > 3.5 and ship_ratio > 3.0 and n_my >= 3 and step > 60:
            self.phase = 'crush'
        elif prod_ratio > 2.0 or ship_ratio > 2.5:
            self.phase = 'aggressive'
        elif n_my >= 3 and prod_ratio > 1.0 and n_enemy > 0 and len(self.neutral_planets) <= 3:
            self.phase = 'dominate'
        elif self.my_prod < enemy_prod * 0.7:
            self.phase = 'defend'
        else:
            self.phase = 'expand'

        # Threat tracking: incoming enemy ship count per owned planet
        ...
        for f in fleets:
            ...
            for mp in self.my_planets:
                ...
                if proj <= 0:
                    continue
                perp_sq = dx * dx + dy * dy - proj * proj
                if perp_sq < (mp.radius + 3) ** 2:
                    self.threats[mp.id] += int(f.ships)

        # Phase-aware defense ratios
        if self.phase == 'crush':
            self.def_ratio = 0.06
            self.stacked_ratio = 0.10
        elif self.phase == 'aggressive':
            self.def_ratio = 0.09
            self.stacked_ratio = 0.13
        elif self.phase == 'defend':
            self.def_ratio = 0.14
            self.stacked_ratio = 0.20
        else:
            self.def_ratio = PROACTIVE_DEFENSE_RATIO
            self.stacked_ratio = STACKED_THREAT_RATIO
    ...
    def aim(self, src_id, tgt_id, ships):
        ...
        if result is not None:
            ...
            if not (0.0 <= ex <= BOARD and 0.0 <= ey <= BOARD):
                ...
                result = None
        # Validate orbiting targets: even exact convergence can fail when
        # waypoint routing (sun avoidance) changes the fleet direction
        if (result is not None
                and not _is_static(tgt, self.initial_by_id)
                and tgt.id not in self.comet_ids):
            angle, turns, tx, ty = result
            if not _validate_orbit_intercept(
                    src, tgt, angle, turns, ships,
...
def _target_value(target, arrival_turns, mission, world):
    ...
    # Distance penalty: penalize far targets (from 1000-score agents)
    # Enders uses -travel*2.5; we scale by remaining to blend with horizon value
    if arrival_turns > 5:
        value -= arrival_turns * target.production * 0.6
...
def _plan_moves(world, deadline=None):
    ...
    for p in world.my_planets:
        ...
        reserve[p.id] = min(int(p.ships), max(keep, proactive))
        budget[p.id] = max(0, int(p.ships) - reserve[p.id])
        # Threat-based offense skip: if under heavy threat, hold garrison
        threat = world.threats.get(p.id, 0)
        if threat > p.ships * 0.30 and world.phase != 'crush':
            budget[p.id] = max(0, budget[p.id] // 2)
...
def agent(obs, config=None):
    """Ultra agent: apex + phase awareness + threat-based defense."""
