...
Builds on the strategic patient agent with more sophisticated decision-making:
  - Prioritizes planets with the most available ships first
  - Breaks ties by proximity to nearest competitor (closer = higher priority)
...
def agent(obs, config=None):
    """
    Advanced agent for Orbit Wars.
    Prioritizes planets with most ships; breaks ties by proximity to competitors.
    ...
    # Sort by available ships (descending); break ties by proximity to nearest
    # competitor (closer planets act first to pressure the enemy)
    competitor_planets = [p for p in planets if p.owner >= 0 and p.owner != player]

    def _nearest_competitor_dist(p):
        if not competitor_planets:
            return float("inf")
        return min(_dist(p.x, p.y, c.x, c.y) for c in competitor_planets)

    my_planets_sorted = sorted(
        my_planets,
        key=lambda p: (-available[p.id], _nearest_competitor_dist(p)),
    )
