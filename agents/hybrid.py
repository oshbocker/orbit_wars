"""
Hybrid agent for Orbit Wars.

Combines the best ideas from three top-performing agents:
- Arrival ledger + timeline simulation for precise ship needs (inspired by
  production-horizon planning approaches)
- Waypoint sun avoidance to preserve strategic options (from net-difference agents)
- Leader targeting in 4-player games (from competitive awareness approaches)
- Mission-based planning: capture, rescue, reinforce, snipe, swarm, evacuate
- Commitment-aware planning with reserve/budget system
- Multi-source coordination for heavily defended targets
- Opening beam search for optimal early game sequencing
"""
...
CRASH_EXPLOIT_BONUS = 1.18
COMET_PENALTY = 0.65
LEADER_BONUS = 1.2

# Ship margins
NEUTRAL_MARGIN_BASE = 2
NEUTRAL_MARGIN_PROD = 2
NEUTRAL_MARGIN_CAP = 8
HOSTILE_MARGIN_BASE = 3
HOSTILE_MARGIN_PROD = 2
...
class World:
    ...
    def __init__(self, player, step, planets, fleets, initial_by_id,
        ...
        self.is_behind = self.domination < -0.20
        self.is_ahead = self.domination > 0.18
        self.is_finishing = (self.domination > 0.35
                             and self.my_prod > sum(self.production.get(o, 0) for o in self.strength if o != player) * 1.25
                             and step > 100)
...
def _target_value(target, arrival_turns, mission, world):
    ...
    if target.owner == -1:
        ...
        value *= OPENING_HOSTILE_MULT if world.is_opening else HOSTILE_VALUE_MULT
...
def _opening_filter(target, arrival_turns, needed, available, world):
    ...
    # Safe high-production rotating neutrals are ok
    my_t = world.my_react.get(target.id, 1e9)
    en_t = world.enemy_react.get(target.id, 1e9)
    if target.production >= 4 and arrival_turns <= 10 and my_t <= en_t - 2:
        return False
    ...
    return arrival_turns > 13 or target.production <= 2
    ...
    for p in world.my_planets:
        ...
        for e in world.enemy_planets:
            ...
            if aim_data[1] > PROACTIVE_DEFENSE_HORIZON:
                continue
            proactive = max(proactive, int(e.ships * PROACTIVE_DEFENSE_RATIO))
...
def _plan_moves(world, deadline=None):
    ...
    if not world.is_late and _time_left() > OPTIONAL_PHASE_MIN_TIME:
        for src in world.my_planets:
            if _expired():
                break
            src_left = _atk_left(src.id)
...
def agent(obs, config=None):
    """Hybrid agent combining best strategies from multiple top agents."""
