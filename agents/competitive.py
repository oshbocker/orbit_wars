"""
Competitive agent for Orbit Wars.

Improves on the lookahead agent by maximizing the **net difference** between
our ships/production and the enemies', rather than just maximizing our own.
Key differences:
  - Net-difference evaluation: score = our_value - enemy_value
  - Enemy aggressive rollout: enemies use simplified production-rush logic
  - 4-player awareness: targets the leading opponent preferentially
  - Same two-pass sequential greedy planner structure as lookahead
"""
...
SHIP_FRACTIONS = (1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4)
ROLLOUT_NEIGHBORS = 5
ROLLOUT_MAX_LAUNCHES = 3

# Competitive-specific
ENEMY_ROLLOUT_PLANETS = 5
ENEMY_ROLLOUT_LAUNCHES = 3
LEADER_BIAS = 1.2
...
class _SimState:
    ...
    def __init__(self, planet_owner, planet_ships, planet_prod, fleet_events,
                 current_step, planet_ids):
        self.planet_owner = planet_owner
        self.planet_ships = planet_ships
        self.planet_prod = planet_prod
...
# ── greedy rollout sub-policy (for our player) ────────────────────────────
...
# ── enemy aggressive rollout (replaces _enemy_decay) ──────────────────────

def _enemy_aggressive_step(state, player, dist_matrix, neighbors):
    """Run simplified aggressive logic for all enemy players.

    For each enemy, find their top planets by ships, pick the best
    production/travel target they don't own, and launch 70% of ships.
    Capped at ENEMY_ROLLOUT_LAUNCHES per enemy per step.
    """
    # Find all enemy player ids
    enemy_players = set()
    for pid in state.planet_ids:
        owner = state.planet_owner[pid]
        if owner >= 0 and owner != player:
            enemy_players.add(owner)

    for enemy in enemy_players:
        enemy_planets = [(pid, state.planet_ships[pid])
                         for pid in state.planet_ids
                         if state.planet_owner[pid] == enemy and state.planet_ships[pid] > 3]
        enemy_planets.sort(key=lambda x: -x[1])
        ...
        for src_id, src_ships in enemy_planets[:ENEMY_ROLLOUT_PLANETS]:
            if launches >= ENEMY_ROLLOUT_LAUNCHES:
                break
            ...
            for target_id in neighbors.get(src_id, [])[:ROLLOUT_NEIGHBORS]:
                if state.planet_owner[target_id] == enemy:
                    continue
                if target_id in targeted:
                    continue

                d = dist_matrix.get((src_id, target_id), 9999.0)
                send_est = src_ships * 0.7
                speed = _fleet_speed(send_est)
                travel = d / max(speed, 0.1)
                ...
                score = target_prod / max(travel, 1.0)
                if score > best_score:
                    best_score = score
                    best_target = target_id
                    best_travel = travel
            ...
            if best_target is not None:
                ...
                state.planet_ships[src_id] -= send
                arrival = state.current_step + max(1, int(math.ceil(best_travel)))
                state.fleet_events.append((arrival, best_target, enemy, send))
...
def _identify_leader(state, player):
    """Find the enemy player with the most ships + production value.

    Returns (leader_id, leader_score) or (None, 0) if no enemies.
    """
    enemy_scores = {}
    for pid in state.planet_ids:
        owner = state.planet_owner[pid]
        if owner >= 0 and owner != player:
            enemy_scores[owner] = enemy_scores.get(owner, 0) + \
                state.planet_ships[pid] + state.planet_prod[pid] * 10

    for _, target_id, owner, ships in state.fleet_events:
        if owner >= 0 and owner != player:
            enemy_scores[owner] = enemy_scores.get(owner, 0) + ships

    if not enemy_scores:
        return None, 0

    leader = max(enemy_scores, key=enemy_scores.get)
    return leader, enemy_scores[leader]


# ── candidate generation & evaluation ──────────────────────────────────────

def _generate_candidates(src_id, state, player, dist_matrix, neighbors, leader_id):
    """Generate WAIT + ATTACK candidates for a source planet.

    Targets owned by the leader get a score bonus (LEADER_BIAS).
    """
...
def _evaluate_state(state, player, leader_id):
    """Net-difference evaluation with leader weighting.

    score = my_value - weighted_enemy_value
    Leader's ships/production are weighted more heavily than other enemies.
    """
    my_ships = 0.0
    my_prod = 0.0
    n_my_planets = 0
    ...
    # Per-enemy tracking
    enemy_ships = {}  # {enemy_id: ships}
    enemy_prod = {}   # {enemy_id: production}

    for pid in state.planet_ids:
        owner = state.planet_owner[pid]
        if owner == player:
            my_ships += state.planet_ships[pid]
            my_prod += state.planet_prod[pid]
            n_my_planets += 1
        elif owner >= 0:
            enemy_ships[owner] = enemy_ships.get(owner, 0) + state.planet_ships[pid]
            enemy_prod[owner] = enemy_prod.get(owner, 0) + state.planet_prod[pid]

    # Fleet ships
    my_fleet_ships = 0.0
    enemy_fleet_ships = {}
    for _, _, owner, ships in state.fleet_events:
        if owner == player:
            my_fleet_ships += ships
        elif owner >= 0:
            enemy_fleet_ships[owner] = enemy_fleet_ships.get(owner, 0) + ships

    remaining = max(0, MAX_STEPS - state.current_step)
    prod_horizon = min(15.0, remaining / 8.0)

    # Our score
    my_score = my_ships + my_fleet_ships + my_prod * prod_horizon + n_my_planets * 2.0

    # Weighted enemy score
    total_enemy_score = 0.0
    all_enemies = set(enemy_ships.keys()) | set(enemy_fleet_ships.keys())
    for eid in all_enemies:
        e_ships = enemy_ships.get(eid, 0) + enemy_fleet_ships.get(eid, 0)
        e_prod = enemy_prod.get(eid, 0)
        e_score = e_ships + e_prod * prod_horizon
        ...
        total_enemy_score += e_score * weight

    return my_score - total_enemy_score * 0.8


def _heuristic_score(src_id, target_id, send, state, dist_matrix, leader_id):
    """Fast heuristic for pre-ranking attack candidates."""
    d = dist_matrix.get((src_id, target_id), 9999.0)
    speed = _fleet_speed(send)
    travel = d / max(speed, 0.1)
    prod = state.planet_prod[target_id]
    score = prod ** 2 / max(travel, 1.0)

    # Bonus for attacking the leader's planets
    if leader_id is not None and state.planet_owner[target_id] == leader_id:
        score *= LEADER_BIAS
...
def _evaluate_candidate(state, src_id, ctype, target_id, send, player,
                        dist_matrix, neighbors, planets_by_id, depth, leader_id):
    """Simulate a candidate action and return its net-difference score."""
    sim = state.copy()
    if ctype == "ATTACK":
    ...
    for _ in range(depth):
        ...
        _enemy_aggressive_step(sim, player, dist_matrix, neighbors)

    return _evaluate_state(sim, player, leader_id)
...
def _run_lookahead(state, player, dist_matrix, neighbors, planets_by_id, depth, time_limit):
    """Two-pass sequential greedy loop with competitive evaluation.

    Same structure as lookahead agent, but uses net-difference scoring
    and enemy aggressive rollouts instead of decay.
    """
    ...
    leader_id, _ = _identify_leader(state, player)

    def _best_action_for(src_id, eval_depth):
        candidates = _generate_candidates(src_id, state, player, dist_matrix,
                                          neighbors, leader_id)
        if len(candidates) <= 1:
            return candidates[0]
        ...
        for ctype, target_id, send in candidates:
            if ctype == "WAIT":
                continue
            h = _heuristic_score(src_id, target_id, send, state, dist_matrix, leader_id)
            attacks.append((h, ctype, target_id, send))
...
def agent(obs, config=None):
    """
    Competitive agent for Orbit Wars.
    Maximizes net advantage (own - enemy) using forward simulation with
    aggressive-style enemy rollouts and leader-targeting in 4-player games.
    """
    t_start = time.time()
