...
def _resolve_config(tag: str, args: argparse.Namespace) -> str:
    ...
    overrides = (
        ...
        or args.log_dir
        or args.demo_cache
        or args.collect_workers is not None
    ...
    print(
        f"  [{tag}] using overridden config -> {out} "
        f"(save_dir={cfg.save_dir}, cache={cfg.imitation.bc_cache_path}, "
        f"collect_workers={cfg.exit.collect_workers})",
        flush=True,
    )
    return str(out)
...
def run_arm(tag: str, args: argparse.Namespace) -> int:
    ...
    print(
        f"=== arm embed{tag} exited code={proc.returncode} in {time.time() - t0:.0f}s ===",
        flush=True,
    )
...
def main() -> int:
    ...
    ap.add_argument(
        "--demo-cache", default=None, help="override BC demo cache path (shared across arms)"
    )
    ap.add_argument(
        "--collect-workers",
        type=int,
